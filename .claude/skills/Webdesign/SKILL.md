...
- **`frontend-design` is AVAILABLE in the official marketplace, not installed.** Verified 2026-09-11: `plugins/installed_plugins.json` holds only code-review, firecrawl and pr-review-toolkit. The earlier wording ("already installed") conflated available with installed, which told the DA not to invoke something that would never fire. Install it before relying on the handoff, or do the frontend work directly. Once installed it auto-activates when the bundle is fed to Claude Code — do NOT manually invoke it then.
