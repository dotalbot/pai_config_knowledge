...
2. **Cluster by pain.** Group the corpus into recurring themes, carrying both how often each recurs AND how much frustration it drew. **Three makes a pattern** — the third manual repetition of the same work is the countable bar at which a theme earns a proposal, so recurrence is a number rather than a judgement call. Below three it needs unusual frustration to qualify; at or above three it is a candidate even when each instance was painless, because the cost is the repetition itself. Frustration still ranks the shortlist; the count only decides admission.
