...
**Source preflight — a source must be live, absent, or blocked, never silent.** Before the fan-out, check each configured source's prerequisites and record a status for every one. A source whose config file or tool is missing is `⏭️ not configured` or `🚫 blocked (<reason>)` in Sources Processed — it is NEVER omitted. Blank output is not evidence a source had nothing; it is usually evidence the source never ran.

```bash
ls skills/Upgrade/youtube-channels.json          # else: YouTube ⏭️ not configured
ls skills/Upgrade/user-sources.json              # else: trending/custom ⏭️ not configured
command -v yt-dlp && command -v gh               # else: that source 🚫 blocked (tool missing)
gh auth status                                   # else: GitHub 🚫 blocked (not authenticated)
```

*(2026-09-11: a full run reported healthy coverage while YouTube and GitHub trending were both dark — the YouTube config had been orphaned in `LIFEOS/RETIRED/PAIUpgrade/` during the PAI→LifeOS migration and `gh` was not installed. Both failed as empty output rather than a reported gap, and neither appeared in the report at all.)*
...
- **A stale channel handle reads as an empty channel.** `@AnthropicAI` returns zero results; the live handle is `@anthropic-ai` (corrected 2026-09-11). When a channel returns nothing while its siblings return videos, suspect the handle before concluding no new uploads.
- **`yt-dlp` writes warnings to stderr that swallow stdout when merged.** Always redirect `2>/dev/null` when piping its JSON, or a version warning will be parsed as the result.
- **Bank seen-state in the same run that fetched it, and distinguish seen from processed.** A listing that was triaged by title is `seen`; only an item whose transcript or README was actually read and assessed is `processed`. Writing `seen` for the whole listing is what keeps the next run from re-presenting the same 20 titles; claiming `processed` for them would silently retire material nobody mined. (2026-09-11: the run that RESTORED both sources ended with `State/youtube-videos.json` and `State/github-trending.json` still `{}` — the same silent-gap class the preflight above exists to catch, one layer down.)
