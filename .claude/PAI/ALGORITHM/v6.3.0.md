...
| **E: Infrastructure** | Any PAI system file changes (hooks, settings, Algorithm, skills) | Re-verify current state of target files before editing — a prior session may have partially implemented something. |
...
### ISA Pivot Check (v2.1 — NEW)

**Before starting any implementation work, re-read the ISA `## Goal` and confirm the work you are about to do still matches it.**

If scope has drifted from the ISA Goal since OBSERVE (user added requirements, discoveries changed direction, a preflight gate revealed a different problem), update the ISA Goal and Criteria NOW before writing code. A pivot mid-EXECUTE without updating the ISA leaves the artifact and the work permanently out of sync.

```
📌 PIVOT CHECK: ISA Goal still matches work about to start? [YES | NO — update ISA Goal first]
```
