...
| **Claude Inference (primary)** | **PRIMARY at every tier for any Inference.ts call (fast/standard/smart/advisor).** OpenCode/GPT-5.5 backend retired 2026-08-20 (archived + shut down). Call without `--opencode`; model tiers resolve to Claude (fast=haiku, standard=sonnet, smart=claude-fable-5). | `bun ~/.claude/PAI/TOOLS/Inference.ts [--level fast\|standard\|smart] <sys> <user>` or `--mode advisor` |
