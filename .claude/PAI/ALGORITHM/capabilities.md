...
| Advisor | VERIFY | **At commitment boundaries on multi-step ISAs.** Before approach commitment, when stuck, once after durable deliverable before declaring done. Skip for short reactive tasks. If empirical results contradict advisor, re-call surfacing the conflict — do NOT silently switch. | `bun ~/.claude/PAI/TOOLS/Inference.ts --opencode --mode advisor <task> <state> <question>` (preferred — GPT-5.5 via OpenCode); fallback without `--opencode` if server unavailable | E3+ |
...
| ParallelDivergentIdeation | BUILD, EXECUTE | Spawn N parallel agents each seeded with a different creative angle, then synthesize into a ranked shortlist. Use when BeCreative and Ideate produce insufficient diversity, when the solution space is genuinely wide-open, or when you need cross-vendor perspective diversity (each agent can use a different model). Overkill for focused execution tasks — reserve for open-ended generation where breadth matters. | `Agent(subagent_type="general-purpose", prompt="Generate [N] ideas from angle: [seed]", run_in_background=true)` × N, then synthesize | E4+ |
...
| **OpenCode Inference (GPT-5.5)** | **PREFERRED at E3+ for any Inference.ts call (fast/standard/smart/advisor).** Dom has more tokens/credits on the OpenCode subscription than Claude. Server at `localhost:7878`, auth via `OPENCODE_SERVER_PASSWORD` in `~/.claude/.env`. Falls back to Claude automatically if unavailable. E1/E2 may stay Claude for latency. | `bun ~/.claude/PAI/TOOLS/Inference.ts --opencode [--level fast\|standard\|smart] <sys> <user>` or `--opencode --mode advisor` |
